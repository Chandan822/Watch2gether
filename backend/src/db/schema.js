...
export const friendRequests = pgTable('friend_requests', {
  ...
  uniqFriendRequest: unique('uniq_friend_request').on(table.senderId, table.receiverId),
...
export const rooms = pgTable('rooms', {
  ...
  roomTypeCode: varchar('room_type_code', { length: 30 }).references(() => roomTypes.code).notNull(),
  visibility: varchar('visibility', { length: 30 }).default('public').notNull(), // 'public', 'private', 'password_protected'
  passwordHash: varchar('password_hash', { length: 255 }), // Used if password_protected
  ownerId: uuid('owner_id').references(() => users.id, { onDelete: 'set null' }),
  videoUrl: text('video_url'),
  videoState: varchar('video_state', { length: 20 }).default('paused').notNull(),
  videoTime: doublePrecision('video_time').default(0).notNull(),
  themeId: uuid('theme_id').references(() => roomThemes.id, { onDelete: 'set null' }),
...
export const roomInvitations = pgTable('room_invitations', {
  ...
  inviteeId: uuid('invitee_id').references(() => users.id, { onDelete: 'cascade' }), // Nullable for generic links
...
export const playbackHistory = pgTable('playback_history', {
  id: uuid('id').defaultRandom().primaryKey(),
  roomId: uuid('room_id').references(() => rooms.id, { onDelete: 'cascade' }).notNull(),
  playedById: uuid('played_by_id').references(() => users.id, { onDelete: 'set null' }),
  videoUrl: text('video_url').notNull(),
  title: varchar('title', { length: 255 }),
...
export const messageReactions = pgTable('message_reactions', {
  ...
  uniqMsgReaction: unique('uniq_msg_reaction').on(table.messageId, table.userId, table.emoji),
