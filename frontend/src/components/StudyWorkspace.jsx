...
export default function StudyWorkspace({ roomId, socket, userRole, onClose }) {
  ...
  useEffect(() => {
    ...
    socket.on('study-init', ({ sharedNotes, activePdfUrl, activePdfPage }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 h-[640px] animate-fade-in text-left">
      ...
      <div className="flex flex-col h-full bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-900 rounded-3xl overflow-hidden shadow-2xl relative">
        ...
        <div className="p-4 bg-slate-50/50 dark:bg-slate-950/20 border-b border-slate-200 dark:border-slate-900 flex flex-col gap-3 shrink-0">
          ...
          {isHostOrCoHost ? (
            ...
            <div className="text-[10px] text-slate-500 italic bg-slate-100 dark:bg-slate-950/20 px-3 py-1.5 border border-slate-200 dark:border-slate-900/60 rounded-xl">
      ...
      <div className="flex flex-col h-full bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-900 rounded-3xl overflow-hidden shadow-2xl relative">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-1.5 border rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                previewMode 
                  ? 'bg-indigo-50 dark:bg-indigo-650/20 border-indigo-200 dark:border-indigo-500/30 text-indigo-600 dark:text-indigo-400' 
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="p-1.5 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/35 text-rose-600 dark:text-rose-450 hover:text-rose-700 dark:hover:text-rose-400 rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center gap-1"
                title="Close Study Notes"
              >
        ...
        {error && (
          <div className="p-2.5 mx-4 mt-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/35 rounded-xl text-[10px] text-rose-600 dark:text-rose-400 font-semibold flex items-center gap-2 animate-fade-in shrink-0">
        ...
        <div className="flex-1 p-4 flex flex-col min-h-0 overflow-y-auto">
          ...
          {!isGuest && (
            <form onSubmit={handleSaveSnapshot} className="flex gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-900/60 shrink-0">
              ...
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow cursor-pointer flex items-center gap-1.5 active:scale-[0.98] shrink-0 font-sans"
              >
          ...
          <div className="mt-5 pt-4 border-t border-slate-200 dark:border-slate-900/80 text-left shrink-0">
            <h4 className="text-[10px] font-extrabold uppercase text-slate-500 tracking-wider mb-3 px-1">Session notes history</h4>
